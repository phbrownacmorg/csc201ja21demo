...
def readParams() -> Tuple[float, float]:
...
def calcFutureval(P:float, rate:float) -> List[float]:
    # Accumulator variable
    values:List[float] = [P]
    while values[-1] < 2 * values[0]: # Last item is less than twice the first one
...
def printTable(P:float, rate:float, amounts:List[float]) -> None:
    ...
    print('Investing $' + str(amounts[0]), 'at', str(rate*100) + "% until it doubles.")
...
def main(args:List[str]) -> int:
    P, rate = readParams() # type: Tuple[float, float]
    amounts:List[float] = calcFutureval(P, rate)
